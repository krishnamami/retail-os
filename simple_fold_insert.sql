-- =====================================================================
-- STEP 5F.3 SIMPLE FOLD INSERT - Direct approach
-- Skip the function complexity, directly compute and insert
-- =====================================================================

-- Set horizon
\set horizon '2026-06-20 10:45:00+00'

-- =====================================================================
-- Step 1: Create applicable properties (all domains)
-- =====================================================================
...
-- =====================================================================
-- Step 2: Compute fold results for all properties
-- =====================================================================
...
-- =====================================================================
-- Step 3: Build property entries as JSONB
-- =====================================================================
...
-- =====================================================================
-- Step 4: Aggregate properties per subject
-- =====================================================================
...
-- =====================================================================
-- Step 5: INSERT into fold_state_snapshot with KB/Policy versions
-- =====================================================================
...
SELECT
    ...
    ss.all_basis_ids,
    (SELECT kb_version FROM runtime.governed_knowledge_base WHERE is_active = true ORDER BY created_at DESC LIMIT 1)::TEXT,
    (SELECT policy_version FROM runtime.governed_policy WHERE is_active = true ORDER BY created_at DESC LIMIT 1)::TEXT,
    CURRENT_TIMESTAMP
...
-- =====================================================================
SELECT '=== FOLD INSERTION RESULTS ===' AS result;

SELECT
    'Total snapshots at horizon' AS check_name,
    COUNT(*)::TEXT AS result
FROM state.fold_state_snapshot
WHERE decision_horizon = '2026-06-20 10:45:00+00'::TIMESTAMPTZ;

SELECT
    'Subject types at horizon' AS check_name,
    string_agg(DISTINCT subject_type || ' (' || COUNT(*)::TEXT || ')', ', ' ORDER BY subject_type)::TEXT AS result
FROM state.fold_state_snapshot
WHERE decision_horizon = '2026-06-20 10:45:00+00'::TIMESTAMPTZ
GROUP BY decision_horizon;

SELECT
    'Product subjects created' AS check_name,
    COUNT(DISTINCT subject_id)::TEXT AS result
FROM state.fold_state_snapshot
WHERE subject_type = 'product'
  AND decision_horizon = '2026-06-20 10:45:00+00'::TIMESTAMPTZ;

SELECT
    'Configuration Request subjects created' AS check_name,
    COUNT(DISTINCT subject_id)::TEXT AS result
FROM state.fold_state_snapshot
...
SELECT
    'S6 subjects (CONFIG-REQ-2026-006 variants)' AS check_name,
    COUNT(DISTINCT subject_id)::TEXT AS result
FROM state.fold_state_snapshot
WHERE subject_type = 'configuration_request'
...
SELECT
    'S7 subject (CONFIG-REQ-2026-007)' AS check_name,
    COUNT(DISTINCT subject_id)::TEXT AS result
FROM state.fold_state_snapshot
WHERE subject_type = 'configuration_request'
