-- =====================================================================
-- STEP 5B.1 SECTIONS 2, 9, 12 READY-TO-PASTE WITH ERROR HANDLING
-- Paste entire script into PGAdmin Query Tool and press F5
-- All results written to temp table, SELECT at end shows everything
-- =====================================================================

-- Create temp results table
CREATE TEMP TABLE IF NOT EXISTS kb_audit_results (
  execution_order INT,
  section_name TEXT,
  status TEXT,
  rows_returned INT,
...
TRUNCATE kb_audit_results;
CREATE TEMP SEQUENCE kb_counter START 1;
...
BEGIN
  BEGIN
    INSERT INTO kb_audit_results (execution_order, section_name, status, rows_returned, result_data)
    SELECT
      nextval('kb_counter'),
  ...
  EXCEPTION WHEN OTHERS THEN
    INSERT INTO kb_audit_results (execution_order, section_name, status, error_message)
    VALUES (nextval('kb_counter'), 'SECTION_2_IDENTITY_INPUT_MATRIX', 'ERROR', SQLERRM);
  END;
END $$;

-- =====================================================================
-- SECTION 9: IDENTITY EFFECT RULES (KB)
-- =====================================================================
DO $$
BEGIN
  BEGIN
    ...
    SELECT
      ...
      string_agg(
        ...
        '|rule_name=' || rule_name || '|description=' || COALESCE(description, 'N/A') ||
        '|status=' || status,
        chr(10)
      )
  ...
  EXCEPTION WHEN OTHERS THEN
    INSERT INTO kb_audit_results (execution_order, section_name, status, error_message)
    VALUES (nextval('kb_counter'), 'SECTION_9_IDENTITY_EFFECT_RULES', 'ERROR', SQLERRM);
  END;
END $$;

-- =====================================================================
-- SECTION 12: CHANGE_CLASSIFICATION RULES (KB)
-- =====================================================================
DO $$
BEGIN
  BEGIN
    ...
    SELECT
      ...
      string_agg(
        ...
        '|description=' || COALESCE(description, 'N/A') || '|status=' || status,
        chr(10)
      )
    FROM claris_kb.identity_rules
  ...
  EXCEPTION WHEN OTHERS THEN
    INSERT INTO kb_audit_results (execution_order, section_name, status, error_message)
    VALUES (nextval('kb_counter'), 'SECTION_12_RULE_INPUT_REQUIREMENTS', 'ERROR', SQLERRM);
  END;
END $$;

-- =====================================================================
-- FINAL OUTPUT - COPY ALL RESULTS BELOW
-- =====================================================================

SELECT '=== EXECUTION SUMMARY ===' AS summary;
SELECT
  execution_order,
  section_name,
  status,
  rows_returned,
  CASE WHEN error_message IS NOT NULL THEN 'ERROR: ' || error_message ELSE 'SUCCESS' END AS result
FROM kb_audit_results
ORDER BY execution_order;

SELECT chr(10) || '=== DETAILED RESULTS ===' AS detail;
SELECT * FROM kb_audit_results ORDER BY execution_order;
