r"""Artifact verification gate for a prototype release (READ ONLY).

Run AFTER publishing at COMPILED and BEFORE promoting to VERIFIED/ACTIVE, then
...
the deployed authoring release it was compiled from.
    ...
    .\venv\Scripts\python.exe tests\decisions\integration\vslice_verify_artifact.py
    .\venv\Scripts\python.exe tests\decisions\integration\vslice_verify_artifact.py 2026.10-prototype.2
...
PRODUCTION_KB = "1.1"
PRIOR_PROTOTYPE = "2026.10-prototype.1"
PRIOR_DIGEST = "888b202b179806c5d5844a0f78739cecae2634d58d6ff106c321e52b5bcde559"
FAILURES: list = []
...
def main(version: str) -> int:
    with read_only_connection() as db:
        ...
        if not rows:
            print(f"   {version} is not published yet. Apply its publish "
                  "migration first.")
            return 2
        stored = rows[0]
        ...
        head("5. EXECUTABLE BINDINGS AS STORED")
        bindings = sorted(payload["sections"]["decision_rule_bindings"],
                          key=lambda r: int(r["precedence"]))
        for b in bindings:
            print(f"   {b['rule_id']:<14} {b['rule_class']:<6} "
                  f"prec={int(b['precedence']):<3} {b['predicate_name']:<46} "
                  f"-> {b['expected_outcome']}")
        check("six bindings", len(bindings), 6)
        check("guards run before any create outcome",
              [b["rule_class"] for b in bindings],
              ["GUARD", "GUARD", "MATCH", "MATCH", "MATCH", "MATCH"])
        check("exact identity match precedes additional configuration",
              [b["rule_id"] for b in bindings if b["rule_id"] in
               ("IA-PRED-004", "IA-PRED-006")],
              ["IA-PRED-004", "IA-PRED-006"])
        check("no FALLBACK",
              [b["rule_id"] for b in bindings if b["rule_class"] == "FALLBACK"],
              [])
        check("projection rules carried", len(payload["sections"]
              ["projection_rules"]), 4)

        head("6. LIFECYCLE AND ISOLATION")
        ...
        check("production never returns a prototype",
        ...
        if stored["status"] == "ACTIVE":
            ...
            check("and so does its validation status",
                  ...
                  "TO_BE_VALIDATED_WITH_CLARIS")
            check("the superseded prototype is ARCHIVED, not deleted",
                  db.scalar("""SELECT status FROM claris_kb.kb_artifact
                               WHERE kb_version=%s""", (PRIOR_PROTOTYPE,)),
                  "ARCHIVED")
        else:
            check("not active yet, so the door still holds the prior release",
                  prototype[0]["kb_version"] if prototype else None,
                  PRIOR_PROTOTYPE)

        head("7. THE PRIOR RELEASE IS UNTOUCHED")
        check("prototype.1 keeps its digest",
              db.scalar("""SELECT content_digest FROM claris_kb.kb_artifact
                           WHERE kb_version=%s""", (PRIOR_PROTOTYPE,)),
              PRIOR_DIGEST)
        check("prototype.1 still recompiles to it",
              compile_release(db, DOMAIN, PRIOR_PROTOTYPE).semantic_digest,
              PRIOR_DIGEST)
