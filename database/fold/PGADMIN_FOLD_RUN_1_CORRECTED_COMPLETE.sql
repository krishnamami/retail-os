-- =====================================================================
-- STEP 4I: CORRECTED FOLD RUN #1 — COMPLETE (WITH DELETE CLEANUP)
-- =====================================================================
-- CRITICAL FIX #1: Changed value_type → property_value_type (correct column name)
-- CRITICAL FIX #2: Added props->>'fold_state' for JSONB key reference
-- CRITICAL FIX #3: Added DELETE statement to clean failed snapshots BEFORE re-execution
-- CRITICAL FIX #4: Fixed V9/V10 ambiguous column reference (assertion_id → basis_id)
-- =====================================================================
  ...
  property_entries AS (
    SELECT
      ...
      (SELECT array_agg(elem::uuid ORDER BY elem::uuid)
       FROM jsonb_array_elements_text(pfr.fold_result->'basis_assertion_ids') AS elem) AS basis_ids
...
-- =====================================================================
-- STEP 1: CLEANUP — DELETE FAILED SNAPSHOTS FROM PREVIOUS RUN
-- =====================================================================
-- This must execute BEFORE Fold Run #1 to ensure clean re-execution
-- Authorization: STEP 4I Clean Failed Run #1

DELETE FROM state.fold_state_snapshot
WHERE decision_horizon = '2026-06-20 10:45:00+00'::timestamptz;

-- =====================================================================
-- STEP 2: EXECUTE FOLD RUN #1 (Re-execute with Corrected Functions)
-- =====================================================================

SELECT * FROM runtime.fold_snapshot_at_horizon('2026-06-20 10:45:00+00'::timestamptz);

-- =====================================================================
-- STEP 3: 12 VALIDATION QUERIES (All Pass/Fail Criteria)
-- =====================================================================
...
-- CRITICAL FIX #4: Changed assertion_id → basis_id to avoid ambiguous column reference
